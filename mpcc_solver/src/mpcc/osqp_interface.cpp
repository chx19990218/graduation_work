...
int OSQPInterface::updateMatrices(
    ...
    Eigen::SparseMatrix<double>& xupp_, int warmStart) {
  // change Q_ upper triangular
  Q_ = Q_.triangularView<Eigen::Upper>();
  ...
  data->P = csc_matrix(data->n, data->n, Q_.nonZeros(), Q_.valuePtr(),
                       Q_.innerIndexPtr(), Q_.outerIndexPtr());
  data->q = new c_float[c_.rows()];
  for (int i = 0; i < c_.rows(); ++i) {
    data->q[i] = c_.coeffRef(i, 0);
  }
  data->A = csc_matrix(data->m, data->n, C_.nonZeros(), C_.valuePtr(),
                       C_.innerIndexPtr(), C_.outerIndexPtr());
...
int OSQPInterface::solveStatus() { return work->info->status_val; }
OSQPSolution* OSQPInterface::solPtr() { return work->solution; }